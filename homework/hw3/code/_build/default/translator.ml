                ...
                |"isinstance" ->
                  (
                    match explst with _::tl ->(
                      match tl with typ::_->(
                        match typ with Spy.Constant(CString(x)) ->
                        (
...
= fun stmt -> match stmt with 
  ...
  | Spy.Assign(explst,exp) -> (
    match explst with 
    | hd::tl -> (translate_assign(hd,exp))@(translate_stmt (Spy.Assign(tl,exp)))
          ...
          (
            ...
            let code = (list_fold
              (fun x c ->
                let l3 =new_label() in  
                let e =new_temp_var() in 
                c@[(l2,ITER_LOAD(e,a,i))]@
                (translate_assign(x,Spy.Name(e)))@
                [(l3,ASSIGNC(i,ADD,i,1))]
              )
              lexplst
              [(l1,COPYC(i,0))]
            ) in 
